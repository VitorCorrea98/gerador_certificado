...
import type { CourseInfo, SignatureConfig, Student } from "./types";
...
function Dashboard({ onLogout }: { onLogout: () => void }) {
	...
	const previewData: Student[] = useMemo(() => {
		...
		return [
			{ Nome: "ADRIANA DA SILVA DUARTE", Funcao: "Apoiadora da linha Materno" },
		];
	}, [students]);
	...
	const handleExcelUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
		...
		reader.onload = (evt) => {
			...
			const cleanData: Student[] = rawData
				.map((row) => {
					const findValue = (keywords: string[]) => {
						const key = Object.keys(row).find((k) =>
							keywords.some((w) => k.toLowerCase().includes(w.toLowerCase())),
						);
						return key ? String(row[key]).trim() : "";
					};
					return {
						Nome: findValue(["nome completo", "nome"]),
						Funcao: findValue(["cargo", "função", "funcao"]) || "Participante",
					};
				})
				.filter((s) => s.Nome !== "");
	...
	const addSignature = () => {
		...
		setSignatures([
			...signatures,
			{ id: crypto.randomUUID(), name: "", role: "", imagePreview: "" },
		]);
	};
	const updateSignature = (
		id: string,
		field: keyof SignatureConfig,
		value: string,
	) => {
		setSignatures((prev) =>
	...
	const handleImageUpload = (id: string, file: File) => {
		updateSignature(id, "imagePreview", URL.createObjectURL(file));
	};
			...
			<div className="w-full lg:w-1/3 space-y-6 overflow-y-auto max-h-screen pb-20">
				...
				<div className="bg-white p-5 rounded-xl shadow-sm border border-gray-100">
					...
					<h2 className="text-sm font-bold flex items-center gap-2 mb-4 text-gray-700">
						<Settings className="w-4 h-4 text-blue-600" /> 1. Informações do
						Curso
					</h2>
					{/* ... Inputs ... */}
					<div className="space-y-3">
						<input
							...
							value={courseInfo.nomeCurso}
							onChange={(e) =>
								setCourseInfo({ ...courseInfo, nomeCurso: e.target.value })
							}
							placeholder="Nome do curso"
						/>
						<input
							className="w-full p-2 border rounded text-sm"
							value={courseInfo.periodo}
							onChange={(e) =>
								setCourseInfo({ ...courseInfo, periodo: e.target.value })
							}
							placeholder="Período"
						/>
						<input
							className="w-full p-2 border rounded text-sm"
							value={courseInfo.cargaHoraria}
							onChange={(e) =>
								setCourseInfo({ ...courseInfo, cargaHoraria: e.target.value })
							}
				...
				<div className="bg-white p-5 rounded-xl shadow-sm border border-gray-100">
					<h2 className="text-sm font-bold flex items-center gap-2 mb-3 text-gray-700">
						<FileSpreadsheet className="w-4 h-4 text-green-600" /> 2. Lista de
						Alunos
					</h2>
					<input
						type="file"
				...
				<div className="bg-white p-5 rounded-xl shadow-sm border border-gray-100">
					...
					<div className="space-y-4">
						{signatures.map((sig) => (
							<div
								key={sig.id}
								className="border border-gray-200 rounded p-3 relative bg-gray-50"
							>
								<button
									type="button"
									onClick={() =>
								...
								<div className="space-y-2">
									<input
										...
										value={sig.name}
										onChange={(e) =>
											updateSignature(sig.id, "name", e.target.value)
										}
									/>
									<input
										placeholder="Cargo"
										...
										value={sig.role}
										onChange={(e) =>
											updateSignature(sig.id, "role", e.target.value)
										}
									/>
									<input
										type="file"
										...
										onChange={(e) =>
											e.target.files &&
											handleImageUpload(sig.id, e.target.files[0])
										}
									/>
								</div>
							</div>
						...
						<button
							...
							className="w-full py-2 border border-dashed border-blue-300 text-blue-600 rounded text-xs hover:bg-blue-50"
						>
			...
			<div className="flex-1 bg-gray-200 rounded-xl overflow-hidden shadow-inner flex flex-col h-[80vh] lg:h-auto">
				<div className="bg-gray-800 text-white p-2 text-xs flex justify-between px-4">
					<span className="flex items-center gap-2">
						<Eye className="w-3 h-3" /> Preview (Apenas 1º aluno)
					</span>
				</div>
